...
public class CapacityPersistenceAdapter implements CapacityPersistencePort {
    ...
    @Override
    public Flux<Capacity> listCapacitiesPage(int page, int size, String sortBy, String sortDir, String messageId) {
        ...
        log.info("consulta ejecutada {}",sql);
        return databaseClient.sql(sql)
                .bind("limit", size )
                .bind("offset", page)
                .map((row, meta) -> {
                       CapacityEntity capacityEntity = new CapacityEntity();
                               capacityEntity.setId(row.get("id", Long.class));
                               capacityEntity.setName(row.get("name", String.class));
                               capacityEntity.setDescription(row.get("description", String.class));
    ...
    public Mono<Boolean> getCapacitiesInOtherBootcamps(Long idBootcamp, String messageId) {
        String sql = """
                select id from capacidades.capacities_x_bootcamps where id_bootcamp <> %s
                    and id_capacity in (select id_capacity  from capacidades.capacities_x_bootcamps where id_bootcamp = %s) limit 1
    ...
    public Flux<BootcampCapacity> getCapacitiesByBootcamp(Long idBootcamp, String messageId) {
        String sql = """
            select id, id_bootcamp, id_capacity from capacidades.capacities_x_bootcamps where id_bootcamp = %s
    ...
    public Mono<Void> deleteCapacitiesByBootcamp(Long idBootcamp, String messageId) {
        // Preparar el SQL
        String sql = """
                DELETE FROM capacidades.capacities WHERE id in (
                    select id_capacity from capacidades.capacities_x_bootcamps where id_bootcamp = %s group by id_capacity)
                """
                .formatted(idBootcamp);
    ...
    public Mono<Void> deleteAllCapacitiesyBootcamp(Long idBootcamp, String messageId) {
        // Preparar el SQL
        String sql = """
                DELETE FROM capacidades.capacities_x_bootcamps WHERE id_bootcamp = %s
                """
                .formatted(idBootcamp);
